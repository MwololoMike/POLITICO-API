...
class TestOfficesEndpoints(TestCase):
    ...
    def test_get_all_offices_successfully(self):
        ...
        response = self.politico_client.get("/offices")
    ...
    def test_get_all_offices_failed(self):
        ...
        response = self.politico_client.get("/offies")
        self.assertTrue(response.status_code == 404)

    def test_get_office_passed(self):
        self.politico_client.post("/offices", data=json.dumps({
            "name": "office_name",
            "type": "office_type"
        }))
        response = self.politico_client.get("/offices/1")
        self.assertTrue(response.status_code == 200)

    def test_get_office_fail(self):
        self.politico_client.post("/offices", data=json.dumps({
            "name": "office_name",
            "type": "office_type"
        }))
        response = self.politico_client.get("/offices/99")
        self.assertTrue(response.status_code == 400)
